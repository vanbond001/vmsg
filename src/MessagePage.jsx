...
const handleClose = (messageId) => {
  ...
  function connectToWs() {
    ...
    const ws = new WebSocket('wss://vchat-backend-cs72.onrender.com');
    setWs(ws);
  ...
  return (
    <div className='flex flex-col  bg-blue-50'>
      ...
      {/* <h1>Message Page for User {selectedUserId}</h1> */}
    ...
    />
      ...
      <div className="overflow-y-scroll absolute top-10 left-3 right-0 bottom-14 mt-16">
        {messages && messages.map((message) => (
          <div key={message._id} className={message.sender === selectedUserId ? 'text-left pr-10 ' : 'text-right pl-10 '}>
            <div
              className={`text-left inline-block p-1 my-2 rounded-md text-sm max-w-72 ${
                message.sender === selectedUserId ? 'bg-white text-gray-500' : 'bg-blue-500 text-white'
              }`}
            >
